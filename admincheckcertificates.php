                        ...
                        <div class="row">
                            <div class="col-12">
                                ...
                                <div class="card-bod table-responsive">  
                                        <?php
                                          $query_run="select courses.course_name,certificates.certificate,certificates.cert_id,user_courses.user_id as userid from courses,user_courses,certificates,userlogin where courses.courseid=user_courses.course_id && user_courses.status='completed' && user_courses.join_id=certificates.join_id && user_courses.user_id=userlogin.user_id && courses.createdby=user_courses.created_by && user_courses.created_by=".$row2['user_id'];
                                ...
                                <table id="datatable" class="table table-bordered nowrap w-100">
                                            ...
                                            <tbody>
                                               ...
                                               if(mysqli_num_rows($sql_query_run) > 0)
                                               {
                                                   ...
                                                   {
                                                    $name=mysqli_query($conn01,"select userdetails.*,userlogin.role from userdetails,userlogin where userdetails.user_id=userlogin.user_id && userdetails.created_by=".$row2['user_id']." && userlogin.user_id=".$rownum['userid']);
                                                    $namedetails=mysqli_fetch_array($name);
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                            <img src="<?php if($namedetails['role']=='customer') { echo "Customers/".$namedetails['photo'];} else if($namedetails['role']=='employee') { echo "Employees/".$namedetails['photo'];} ?>" alt="" class="avatar-sm rounded-circle me-2">
                                            <a href="#" class="text-body"><?php echo $namedetails['name']; ?></a> <code class="highlighter-rouge">(<?php echo ucfirst($namedetails['role']) ?>)</code>
                                            </td>
                                                <td>
